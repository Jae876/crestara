...
export default function WithdrawalsPage() {
  ...
  const fetchWithdrawals = useCallback(() => {
    setLoading(true);
    fetch(`/api/admin/withdrawals?status=${statusFilter}`, { headers: { Authorization: `Bearer ${token}` } })
      .then((r) => r.json())
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
        {['PENDING', 'CONFIRMED', 'FAILED'].map((s) => (
          <button key={s} onClick={() => setStatusFilter(s)}
            style={{ padding: '6px 14px', borderRadius: 8, fontFamily: 'Orbitron, system-ui', fontSize: '0.7rem', fontWeight: 700, cursor: 'pointer', letterSpacing: '0.06em', border: '1px solid', transition: 'all 0.15s',
              background: statusFilter === s ? statusColors[s] + '20' : 'transparent',
              color: statusFilter === s ? statusColors[s] : '#4a5a6a',
              borderColor: statusFilter === s ? statusColors[s] + '50' : '#1a3050',
            }}>
            {s}
          </button>
        ))}
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16, flexWrap: 'wrap' }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                    <span style={{ fontFamily: 'Orbitron, system-ui', fontSize: '1.1rem', fontWeight: 900, color: '#ffd700' }}>
                      ${w.amountUSD.toFixed(2)}
                    </span>
                    <span style={{ fontFamily: 'Orbitron, system-ui', fontSize: '0.7rem', color: '#c9a96e', background: 'rgba(201,169,110,0.1)', padding: '2px 8px', borderRadius: 6, border: '1px solid rgba(201,169,110,0.25)' }}>
                      {w.coinSymbol}
                    </span>
                    <span style={{ fontSize: '0.7rem', fontWeight: 700, padding: '2px 8px', borderRadius: 6, fontFamily: 'Orbitron, system-ui', background: statusColors[w.status] + '15', color: statusColors[w.status], border: `1px solid ${statusColors[w.status]}40` }}>
                      {w.status}
                    </span>
                  </div>
                  ...
                  <div style={{ fontFamily: 'monospace', fontSize: '0.75rem', color: '#4a5a6a', wordBreak: 'break-all' }}>
                    To: {w.destinationAddress || 'N/A'}
                  </div>
                  <div style={{ fontSize: '0.7rem', color: '#3a4a5a', marginTop: 4 }}>
