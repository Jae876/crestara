...
export default function UsersPage() {
  ...
  const fetchUsers = useCallback(() => {
    setLoading(true);
    fetch(`/api/admin/users?page=${page}&search=${encodeURIComponent(search)}`, { headers: { Authorization: `Bearer ${token}` } })
      .then((r) => r.json())
  ...
  return (
    <AdminShell>
      ...
      <p style={{ color: '#6b7e96', fontSize: '0.85rem', marginBottom: 20 }}>Total: {total.toLocaleString()} users</p>
      ...
      {loading ? (
        ...
        <div style={{ background: 'rgba(13,32,64,0.6)', border: '1px solid #1a3050', borderRadius: 12, overflow: 'hidden' }}>
              ...
              <thead>
                <tr style={{ background: 'rgba(6,13,23,0.6)' }}>
                  {['Email', 'Balance', 'Role', 'KYC', 'Bets', 'Bots', 'Joined', 'Actions'].map((h) => (
                    <th key={h} style={{ padding: '10px 14px', textAlign: 'left', color: '#4a5a6a', fontFamily: 'Orbitron, system-ui', fontSize: '0.65rem', letterSpacing: '0.08em', textTransform: 'uppercase', fontWeight: 600 }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {users.map((u, i) => (
                  <tr key={u.id} style={{ borderTop: '1px solid rgba(26,48,80,0.4)', background: i % 2 === 0 ? 'transparent' : 'rgba(13,32,64,0.2)' }}>
                    ...
                    <td style={{ padding: '10px 14px' }}>
                      <span style={{ fontSize: '0.65rem', fontWeight: 700, padding: '2px 6px', borderRadius: 5, fontFamily: 'Orbitron, system-ui', background: u.role === 'ADMIN' ? 'rgba(245,101,101,0.15)' : 'rgba(0,196,180,0.1)', color: u.role === 'ADMIN' ? '#f56565' : '#00c4b4', border: `1px solid ${u.role === 'ADMIN' ? 'rgba(245,101,101,0.3)' : 'rgba(0,196,180,0.2)'}` }}>
                        {u.role}
                      </span>
                    </td>
                    <td style={{ padding: '10px 14px' }}>
                      <span style={{ fontSize: '0.65rem', padding: '2px 6px', borderRadius: 5, background: 'rgba(26,48,80,0.5)', color: '#6b7e96' }}>{u.kycStatus}</span>
                    </td>
                    ...
                    <td style={{ padding: '10px 14px' }}>
                      <button onClick={() => { setEditing(u.id); setEditBalance(u.balanceUSD.toString()); setMsg(''); }}
                        style={{ padding: '4px 10px', borderRadius: 6, background: 'rgba(201,169,110,0.1)', color: '#c9a96e', fontSize: '0.7rem', cursor: 'pointer', border: '1px solid rgba(201,169,110,0.25)', fontFamily: 'Orbitron, system-ui', fontWeight: 700 }}>
                        Edit
                      </button>
                    </td>
                  </tr>
          ...
          {total > 25 && (
            <div style={{ display: 'flex', justifyContent: 'center', gap: 8, padding: '14px', borderTop: '1px solid #1a3050' }}>
              <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1}
                style={{ padding: '6px 14px', borderRadius: 8, background: 'rgba(13,32,64,0.7)', color: '#6b7e96', fontSize: '0.75rem', cursor: 'pointer', border: '1px solid #1a3050' }}>← Prev</button>
              ...
              <button onClick={() => setPage((p) => p + 1)} disabled={page >= Math.ceil(total / 25)}
                style={{ padding: '6px 14px', borderRadius: 8, background: 'rgba(13,32,64,0.7)', color: '#6b7e96', fontSize: '0.75rem', cursor: 'pointer', border: '1px solid #1a3050' }}>Next →</button>
            </div>
          )}
        </div>
      )}
